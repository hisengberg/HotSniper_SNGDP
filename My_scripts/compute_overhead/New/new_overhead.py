...
import average_mhd_distance_overhead as amd  # when total_cores changed, remember modified in this code.
...
def gdp_map():    # The function to find the GDP optimized active core map.
    ...
    if taskCoreRequirement > n_ipc:
        ...
        # 创建一个for循环，遍历每一个core，求得每一个core的AMD值AMD_core(i)，根据设定的AMDmax，若AMD_core(i) > AMDmax，则将core(i)指定为unavailable.
        for i in range(0, idx_available_cores.shape[0]):
            # 算出每个core的AMD值
            AMD = amd.compute_core_amd(i)
            if AMD > AMDmax:    # 如果该core的AMD值大于所设定的AMDmax，那么该core就不在后续的考虑范围内
                idx = i
                availableCores[idx_available_cores[idx]] = False  # 将大于AMDmax的core设为available False
    ...
    print('======================================================')
    print('New_map runtime overhead: {} ms'.format(execution_time))
    print('======================================================')

    print('[Scheduler] [New]: New determined cores to activate: ', cores_to_activate)
...
def gdp_power(A, core_map, temp_max, temp_amb, P_s, P_k, T_c, gdp_mode):
    ...
    print('=============================================================')
    print('New_power runtime overhead: {} ms'.format(execution_time))
    print('=============================================================')
