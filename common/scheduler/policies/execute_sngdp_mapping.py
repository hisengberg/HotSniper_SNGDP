...
def execute_sngdp_mapping(taskCoreRequirement):
    print('[Scheduler] [SNGDP]: Starting the SNGDP mapping process by executing execute_sngdp_mapping.py')
    ...
    # load the mapping information from file info_for_mapping.txt, saved in mapSNGDP::map in mapSNGDP.cc
    ...
    # compute the new active core indexes using sngdp_mapping
    cores_to_activate = sngdp.sngdp_map(A, temp_max, temp_amb, taskCoreRequirement, activeCores, availableCores, preferredCoresOrder, P_s)

    print('[Scheduler] [SNGDP]: SNGDP determined cores to activate: ', cores_to_activate)

    # write the SNGDP mapping results to file
    file_sngdp_map = open('./system_sim_state/sngdp_map.txt', 'w')
    for core in cores_to_activate:
        file_sngdp_map.write(str(int(core)) + ' ')
    file_sngdp_map.close()

    # -------------------------------------------------------------------------------------------------
    # draw the SNGDP mapping figure to file
...
execute_sngdp_mapping(sys.argv[1])
