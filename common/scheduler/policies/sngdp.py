...
def sngdp_map(A, temp_max, temp_amb, taskCoreRequirement, activeCores, availableCores, preferredCoresOrder, P_s):
    # The function to find the SNGDP optimized active core map.
    ...
    # preferredCoresOrder: the user specified activation order vector, -1 indicates the end of the preferred cores. These cores should be activated first (if available) before SNGDP computation. If user do not specify any activation order, simply set all elements to -1.
    ...
    core_num = availableCores.shape[0]  # .shape[0]: Read the length of the first dimension of the matrix.

    # find the user specified preferred cores that are still in available cores, they should be activated first without SNGDP computing
    ...
    # If taskCoreRequirement <= n_ipc, then we are simply done without SNGDP computation. Otherwise (if taskCoreRequirement > n_ipc), we need to determine the remaining active cores using SNGDP iterations.
    if taskCoreRequirement > n_ipc:
        # initiate SNGDP iterations
        ...
        # --------------------------------------------------------------------------------------------------------------
        current_dir = os.getcwd()  # <current_dir = /home/gjc/HotSniper/benchmarks>
        AMDmax_path = os.path.join(current_dir, 'system_sim_state', 'AMDmax.txt')
        try:
            with open(AMDmax_path, 'r') as AMDmax_file:
                AMDmax = AMDmax_file.read()
                if AMDmax:
                    print('================= MAMD = {} ============================'.format(AMDmax))
                else:
                    print('================ AMDmax_file is empty ================')
        except FileNotFoundError:
            print('================== AMDmax_file is not found =================')
        except IOError:
            print('=============== An IO error occurred while reading the AMDmax_file ===============')
        ...
        # Create a for loop, traverse each core, and obtain the AMD value AMD_core(i) of each core. According to the set AMDmax, if AMD_core(i) > AMDmax, specify core(i) as unavailable.
        for i in range(0, idx_available_cores.shape[0]):
            # compute each core's AMD value 
            AMD = amd.compute_core_amd(i)
            if AMD > AMDmax:    # If the AMD value of the core is greater than the set AMDmax, then the core will not be included in the core candidates.
                idx = i
    ...
    print('======================================================')
    print('SNGDP task_mapping runtime overhead: {} ms'.format(execution_time))
    print('======================================================')

    return cores_to_activate


def sngdp_power(A, core_map, temp_max, temp_amb, P_s, P_k, T_c, sngdp_mode):
    # The function to compute the SNGDP power budget for a given active core map
    # Inputs:
    ...
    # gdp_mode: 'steady' for steady state SNGDP and 'transient' for transient SNGDP
    # Output:
    ...
    print('=============================================================')
    print('SNGDP power_budgeting runtime overhead: {} ms'.format(execution_time))
    print('=============================================================')
