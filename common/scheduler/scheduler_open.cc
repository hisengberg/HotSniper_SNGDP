...
/* add for coldestCore begin */
#include "policies/coldestCore.h"
/* add for coldestCore begin */

/* add for AMDCore begin */
#include "policies/AMDCore.h"
/* add for AMDCore end */
...
/* add for sngdp begin */
#include "policies/dvfsSNGDP.h"
#include "policies/mapSNGDP.h"
/* add for sngdp end */
...
void SchedulerOpen::initMappingPolicy(String policyName) {
	...
	} /* add for coldestCore end */
	/* add for AMDCore begin */
	else if (policyName == "AMDCore") {
		vector<int> preferredCoresOrder;
		for (core_id_t core_id = 0; core_id < (core_id_t)Sim()->getConfig()->getApplicationCores(); core_id++)
		{
			int p = Sim()->getCfg()->getIntArray("scheduler/open/preferred_core", core_id);
		...
		mappingPolicy = new AMDCore(coreRows, coreColumns, preferredCoresOrder);
	} /* add for AMDCore end */
	/* add for gdp begin */
	...
	/* add for sngdp begin */
	    ...
	    mappingPolicy = new MapSNGDP(coreRows, coreColumns, preferredCoresOrder);	
	} /* add for sngdp end */
...
void SchedulerOpen::initDVFSPolicy(String policyName) {
	...
	/* add for sngdp begin */
	  ...
	  {
	    dvfsPolicy = new DVFSSNGDP(thermalModel, performanceCounters, coreRows, coreColumns, minFrequency, maxFrequency, frequencyStepSize);
	  } //Place to instantiate a new DVFS logic. Implementation is put in "policies" package.
	/* add for sngdp end */	
...
bool SchedulerOpen::executeMappingPolicy(int taskID, SubsecondTime time) {
	...
	if ((int)bestCores.size() < openTasks[taskID].taskCoreRequirement) {
		// Guo add begin
		cout << "bestCores.size() = [" << (int)bestCores.size() << "]  <  openTasks[taskID].taskCoreRequirement = [" << openTasks[taskID].taskCoreRequirement << "]" << endl;
		// Guo add end
...
void SchedulerOpen::periodic(SubsecondTime time) {
	if (time.getNS () % 1000000 == 0) { //Error Checking at every 1ms. Can be faster but will have overhead in simulation time.
		cout << "\n-------------------------------------------------new period begin-------------------------------------------------" << endl;
		cout << "\n[Scheduler]: Time " << formatTime(time) << " [Total Cores = " << numberOfCores << " | Active Tasks =  " << numberOfActiveTasks () << " | Completed Tasks = " <<  numberOfTasksCompleted () << " | Queued Tasks = "  << numberOfTasksInQueue () << " | Non-Queued Tasks  = " <<  numberOfTasksWaitingToSchedule () <<  " | Free Cores = " << numberOfFreeCores () << " | Active Tasks Requirements = " << totalCoreRequirementsOfActiveTasks () << " ]" << endl;
	...
	if ((dvfsPolicy != NULL) && (time.getNS() % dvfsEpoch == 0)) {
		...
		// find the active core mapping, store in threadMapping, and write into file
		// note that only the cores assigned to threads are active, the cores only assigned to task but not to thread are not active
		if (typeid(*dvfsPolicy).name() == typeid(DVFSSNGDP).name())
		{
		    std::vector<bool> threadMapping(numberOfCores,false);
		    for(int coreCounter=0;coreCounter<numberOfCores;coreCounter++)
		    {
